...
try:
    ...
    for symbol in symbols:
        ...
        s3_location = f"s3://{BUCKET}/gold/ticks/{symbol}/"

        # Create Iceberg table (if not exists)
        print(f"Creating Iceberg table for {symbol}...")
        ...
        WHEN NOT MATCHED THEN
            INSERT (DateTime, Bid, Ask, symbol, year)
            VALUES (s.DateTime, s.Bid, s.Ask, s.symbol, s.year)
        """
    ...
    print(f"Per‑symbol Iceberg tables: {', '.join([f'ticks_{s.lower()}' for s in symbols])}")
