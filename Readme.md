...
Once financial data is sitting in partitioned Iceberg tables on S3, a second problem shows up: different roles need to access that same Gold dataset in different ways, and locking everyone into a single query engine doesn't work well in practice. A Data Analyst running ad-hoc SQL doesn't want to spin up a Docker cluster, a Data Engineer doing heavier local analytics doesn't want to pay for cloud compute on every query, and an ML Engineer just needs read access to pull training data, not a full query interface at all.

This project was built to solve that access problem directly. It's a lakehouse layer where a single, versioned, partitioned Gold dataset in Iceberg can be queried from multiple independent engines, cloud and on-premises, without duplicating the data or maintaining separate pipelines per engine. Specifically, it needed to:

- Store the Gold dataset once, in a format (Iceberg on S3) that supports ACID guarantees, partitioning, and multiple concurrent readers
- Expose that same dataset to a serverless cloud query engine (Athena) for teams that want SQL without managing infrastructure
- Expose it to on-premises engines (Trino and Spark, via Docker) for teams that want local, high-performance or large-scale distributed querying
- Enforce access per role (Data Engineer/Admin, Data Engineer/Analyst, ML/AI Engineer) through S3 and Glue IAM policies, rather than per query engine
- Keep an ML/AI training path available off the same Gold data, separate from the three query engines

Everything below, from ingestion through the Glue ETL job and the three query engines, is the system built to solve that problem end to end.
